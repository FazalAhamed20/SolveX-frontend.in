...
interface PlanProps {
  ...
  const Plan: React.FC<PlanProps> = ({ name, price, features, isBestValue, icon, interval,_id }) =>{
  ...
  return (
    ...
    <div className={`bg-white rounded-2xl shadow-xl p-6 flex flex-col h-full transform transition duration-300 hover:scale-105 ${isBestValue ? 'border-4 border-green-500' : ''}`}>
      {isBestValue && (
      ...
      <ul className="flex-grow mb-6 space-y-3">
        {(features ?? []).map((feature, index) => (
          <li key={index} className={`flex items-center ${feature.included ? 'text-gray-800' : 'text-gray-400'}`}>
            {feature.included ? (
      ...
      <button className="bg-green-500 hover:bg-green-600 text-white font-bold py-3 px-6 rounded-lg transition duration-200 shadow-lg hover:shadow-xl"
...
const SubscriptionComponent: React.FC = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-16 bg-gradient-to-br">
      <h2 className="text-4xl font-bold text-center mb-4 text-gray-800">Choose Your Perfect Plan</h2>
      <p className="text-center text-gray-600 mb-12">Unlock your coding potential with our flexible subscription options</p>
      <div className="flex justify-center mb-12">
        <div className="bg-white rounded-full p-1 shadow-md">
          <button
            className={`px-6 py-2 rounded-full text-sm font-medium transition duration-200 ${interval === 'monthly' ? 'bg-green-500 text-white' : 'text-gray-600'}`}
            onClick={() => setInterval('monthly')}
          >
            Monthly
          </button>
          <button
            className={`px-6 py-2 rounded-full text-sm font-medium transition duration-200 ${interval === 'yearly' ? 'bg-green-500 text-white' : 'text-gray-600'}`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {plans.map((plan) => (
          <Plan
            key={plan._id} 
            name={plan.title}
            price={interval === 'monthly' ? (plan.monthlyPrice ?? 0) : (plan.yearlyPrice ?? 0)} 

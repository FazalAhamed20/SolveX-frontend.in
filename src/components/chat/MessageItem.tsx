      ...
      >
        {isOwnMessage && showDropdown && (
         ...
         <button
           ...
           className='flex items-center px-4 py-2 w-full text-xs sm:text-sm text-red-600 hover:bg-red-100 rounded-md z-10'
         >
            ...
            <button
            onClick={handleReply}
            className='flex items-center px-4 py-2 w-full text-xs sm:text-sm text-blue-600 hover:bg-blue-100 rounded-md'
          >
...
{!isOwnMessage && (
        <div className='w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-green-100 flex-shrink-0 mr-2 overflow-hidden shadow-md self-end'>
          {message.sender.avatar ? (
            ...
            />
              ...
              <div className='w-full h-full flex items-center justify-center text-xs sm:text-sm font-semibold text-green-600'>
      ...
      <div
        className={`relative max-w-[85%] sm:max-w-[75%] md:max-w-[65%] px-3 py-2 sm:px-4 sm:py-2 rounded-lg shadow-md ${
          isOwnMessage
      ...
      >
          {!isOwnMessage && (
           ...
           <span className='text-xs font-bold text-green-700 mr-4 sm:mr-6'>
             {message.sender.name}
           </span>
           <button
             onClick={handleReply}
...
{message.voice && (
          <div className='flex items-center space-x-2 mb-2'>
            <button
              onClick={toggleAudio}
              className='p-1 sm:p-2 bg-green-500 text-white rounded-full hover:bg-green-600 transition-colors duration-200'
            >
...
<div className='pr-6 sm:pr-8'>
          ...
          {reactions.length > 0 && (
            ...
            >
              {Object.entries(reactionSummary).map(([emoji, count]) => (
                <div
                  key={emoji}
                  className='bg-white shadow-md px-1 py-0.5 sm:px-2 sm:py-1 rounded-full text-[10px] sm:text-xs'
                >
                  {emoji} {count}
                </div>
              ))}
              {Object.keys(reactionSummary).length < reactions.length && (
                <div className='bg-white shadow-md px-1 py-0.5 sm:px-2 sm:py-1 rounded-full text-[10px] sm:text-xs'>
          ...
          <div className='flex justify-end items-center mt-1 space-x-1'>
            <span className='block text-[10px] sm:text-[11px] text-green-600'>
              {formattedTime}
            </span>
            {renderMessageStatus}
          </div>
        ...
        <div
          className={`absolute w-2 h-2 sm:w-3 sm:h-3 ${
            isOwnMessage
              ? 'right-0 -mr-1 sm:-mr-1.5 bg-green-100'
              : 'left-0 -ml-1 sm:-ml-1.5 bg-white'

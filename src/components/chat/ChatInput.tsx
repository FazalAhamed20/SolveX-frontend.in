...
const ChatInput: React.FC<ChatInputProps> = ({
  ...
  return (
    <div className='bg-white border-t border-[#c8e6c9] p-2 sm:p-4 shadow-lg'>
        ...
        <div className='flex flex-wrap items-center gap-2 sm:gap-3'>
          <div className='flex items-center space-x-2 sm:space-x-3'>
            ...
            <button
              ...
              className={`text-white p-2 rounded-full transition-all duration-300 shadow-md hover:shadow-lg ${
          ...
          <div className='flex-1 min-w-0'>
            {isRecording ? (
              <div className="flex items-center justify-center w-full">
                <div className="bg-red-500 text-white px-3 py-1 rounded-full animate-pulse text-xs sm:text-sm">
                  Recording
                </div>
              </div>
            ) : voiceReady ? (
              <div className="flex items-center justify-center w-full">
                <div className="bg-green-500 text-white px-3 py-1 rounded-full text-xs sm:text-sm">
                  Voice Ready
                </div>
              </div>
            ) : (
              <div className="w-full">
                {replyTo && (
                  <div className="bg-gray-100 p-2 rounded-md mb-2 text-xs sm:text-sm">
                    <div className="flex justify-between items-center mb-1">
                      <span className="font-semibold truncate">Replying to {replyTo.sender.name}</span>
                      <button 
                        onClick={clearReply} 
                        className="text-gray-600 hover:text-gray-800 transition-colors duration-200 ml-2"
                      >
                        <FaTimes size={14} />
                      </button>
                    </div>
                    <div className="flex items-center">
                      ...
                      {replyTo.voice && (
                        <div className="mr-2">
                          <FaMicrophone size={14} className="text-blue-500" />
                        </div>
                      )}
                      <p className="text-gray-600 truncate flex-1 text-xs">
                ...
                <input
                  ...
                  className="w-full border-2 border-[#a5d6a7] rounded-full px-3 py-1 sm:px-4 sm:py-2 focus:outline-none focus:ring-2 focus:ring-[#4caf50] transition-all duration-300 text-sm"
          ...
          <button
            type='submit'
            className='bg-[#4caf50] text-white p-2 rounded-full hover:bg-[#43a047] transition-all duration-300 shadow-md hover:shadow-lg'
          >
      ...
      {showEmojiPicker && (
        <div className='absolute bottom-16 left-2 z-10 sm:bottom-20 sm:left-4'>

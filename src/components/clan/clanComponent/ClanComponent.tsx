...
import { FaUsers, FaTrophy, FaPlus, FaTimes, FaSearch, FaFilter, FaTable, FaTh } from 'react-icons/fa';
...
const ClanComponent: React.FC = () => {
  ...
  const handleCreateClan = async (name: string, description: string) => {
    ...
    if (response.payload) {
      setClans(prevClans => [...prevClans, response.payload as unknown as Clan]);
    }
console.log("clans",clans)
  ...
  const renderGridView = () => (
    ...
    >
      {filteredClans.map((clan) => (
        ...
        >
          <div className="p-6">
            <h2 className="text-2xl font-semibold mb-2 text-green-600">{clan.name}</h2>
            <p className="text-gray-600 mb-4 h-12 overflow-hidden">{clan.description}</p>
          ...
          <div className="bg-green-50 px-6 py-3">
            <button className={`w-full py-2 rounded-lg transition-colors duration-300 ${
              isUserMember(clan) ? 'bg-green-600 text-white' : 'bg-white text-green-600 border border-green-600'
            }`}>
  ...
  return (
    <div className="container mx-auto p-4 sm:p-8 bg-gray-100 min-h-screen">
      <div className="mb-8">
        <h1 className="text-3xl sm:text-4xl font-bold text-center text-green-600 mb-4">Coding Clans</h1>
        <p className="text-center text-gray-600">Join forces with fellow coders and conquer challenges together!</p>
      </div>
        ...
        <div className="flex flex-wrap justify-center sm:justify-end space-x-2 sm:space-x-4">
          <button
            ...
            className={`flex items-center px-4 py-2 rounded-lg transition-colors duration-300 ${
              showMyClans ? 'bg-green-600 text-white' : 'bg-white text-green-600 border border-green-600'
            }`}
          >
          ...
          >
            {viewMode === 'grid' ? <FaTable className="mr-2" /> : <FaTh className="mr-2" />}
      ...
      {filteredClans.length === 0 ? (
        <div className="text-center py-12">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">No clans found</h3>
          <p className="mt-1 text-sm text-gray-500">Get started by creating a new clan.</p>
        </div>
      ) : (
          ...
          >
            ...
            >
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-2xl font-bold text-green-600">{selectedClan.name}</h2>
                <button onClick={() => setShowClanModal(false)} className="text-gray-500 hover:text-gray-700">
                  <FaTimes />
                </button>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  <FaTrophy className="text-yellow-500 mr-2" />
                  <span className="text-gray-600">{selectedClan.trophies} trophies</span>
                </div>
                <button className={`px-4 py-2 rounded-lg transition-colors duration-300 ${
                  isUserMember(selectedClan) ? 'bg-green-600 text-white' : 'bg-white text-green-600 border border-green-600'
                }`}
                onClick={isUserMember(selectedClan) ? handleEnterClan : handleJoinClan}>

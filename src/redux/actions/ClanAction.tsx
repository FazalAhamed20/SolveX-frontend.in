...
>(
  'clan/createClan',
  async (clanData: Clan, { rejectWithValue }) => {
    try {
      console.log("clanData",clanData);

      const { data } = await ClanAxios.post<VerifyResponse>(
        '/create-clan',
        {
            id:clanData.id,
            name:clanData.name,
...
>(
  ...
  async ({ clanId, _id,memberName }, { rejectWithValue }) => {
    try {
      ...
      // Prepare the request payload
      const response = await ClanAxios.delete(`/clans/${clanId}/members/${_id}/${memberName}`);

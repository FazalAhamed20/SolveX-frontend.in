...
import { FaPlus, FaTimes, FaDollarSign, FaClipboardList, FaCrown } from 'react-icons/fa';
...
interface SubmitSubscription extends Omit<Subscription, 'features' | 'monthlyPrice' | 'yearlyPrice'> {
  features: string[];
  monthlyPrice: number;
  yearlyPrice: number;
}
...
const AddSubscriptionModal: React.FC<AddSubscriptionModalProps> = ({ isOpen, onClose, onAddSuccess, }) => {
  ...
  const validateField = (name: keyof Subscription, value: string): string => {
    switch (name) {
      ...
      case 'tier':
        return ['basic', 'pro', 'premium'].includes(value) ? '' : 'Invalid tier selected';
      default:
        return '';
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isFormValid) {
      const submitSubscription: SubmitSubscription = {
        ...
        features: newSubscription.features.split(',').map(feature => feature.trim()).filter(feature => feature !== '')
      };
          ...
          <div>
            ...
            <input
              ...
              className={`mt-1 block w-full rounded-md shadow-sm focus:ring focus:ring-opacity-50 ${
                errors.title ? 'border-red-300 focus:border-red-300 focus:ring-red-200' : 'border-gray-300 focus:border-blue-300 focus:ring-blue-200'
              }`}
              required
            />
              ...
              <div className='mt-1 relative rounded-md shadow-sm'>
                ...
                <input
                  ...
                  className={`block w-full pl-7 pr-12 rounded-md focus:ring focus:ring-opacity-50 ${
                    errors.monthlyPrice ? 'border-red-300 focus:border-red-300 focus:ring-red-200' : 'border-gray-300 focus:border-blue-300 focus:ring-blue-200'
                  }`}
                  placeholder='0.00'
                  required
                />
            ...
            <div className='flex-1'>
              ...
              <div className='mt-1 relative rounded-md shadow-sm'>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  className={`block w-full pl-7 pr-12 rounded-md focus:ring focus:ring-opacity-50 ${
                    errors.yearlyPrice ? 'border-red-300 focus:border-red-300 focus:ring-red-200' : 'border-gray-300 focus:border-blue-300 focus:ring-blue-200'
                  }`}
                  placeholder='0.00'
                  required
                />
            ...
            <div className='mt-1 relative rounded-md shadow-sm'>
              ...
              <textarea
                ...
                className={`block w-full pl-10 rounded-md focus:ring focus:ring-opacity-50 ${
                  errors.features ? 'border-red-300 focus:border-red-300 focus:ring-red-200' : 'border-gray-300 focus:border-blue-300 focus:ring-blue-200'
                }`}
            ...
            <div className='mt-1 relative rounded-md shadow-sm'>
              ...
              <select
                ...
                className={`block w-full pl-10 pr-10 py-2 text-base focus:outline-none focus:ring sm:text-sm rounded-md ${
                  errors.tier ? 'border-red-300 focus:border-red-300 focus:ring-red-200' : 'border-gray-300 focus:border-blue-500 focus:ring-blue-500'
                }`}
                required
              >
